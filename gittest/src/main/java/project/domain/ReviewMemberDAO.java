...
public class ReviewMemberDAO {
	...
	// 댓글 기능 구현
		...
		public int deleteReview(int review_no) {
			...
			try {
				// mapper.xml의 id값
				cnt = sqlSession.delete("deleteReview", review_no);
				
				if(cnt > 0) {
					// 만약에 내가 원하는 일을 했으면 DB에 반영
					System.out.println("DAO : 리뷰삭제 성공!!");
					sqlSession.commit();
				} else {
					// 만약에 원하는 일을 못하면 다시 원래대로 돌려주기
					sqlSession.rollback();
		...
		// 좋아요정보 수정 기능 구현
				public int updateFavor(ReviewMember updateFavor) {
					int cnt = 0;
					try {
						// mapper.xml의 id값
						cnt = sqlSession.update("updateFavor", updateFavor);
